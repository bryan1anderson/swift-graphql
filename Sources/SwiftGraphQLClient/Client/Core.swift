...
public final class Client: GraphQLClient, ObservableObject {
    ...
    nonisolated(unsafe) private let operations = PassthroughSubject<Operation, Never>()

    /// Stream of results that may be used as the base for sources.
    nonisolated(unsafe) private var _results: AnyPublisher<OperationResult, Never>
    nonisolated(unsafe) private var results: AnyPublisher<OperationResult, Never> {
        get {
            lock.lock()
            defer { lock.unlock() }
            return _results
    ...
    nonisolated(unsafe) private var _active: [String: Source]
    nonisolated(unsafe) private var active: [String: Source] {
        get {
            lock.lock()
            defer { lock.unlock() }
            return _active
    ...
    nonisolated(unsafe) private var _cancellables = Set<AnyCancellable>()
    ...
    private func createResultSource(operation: Operation) -> Source {
        ...
        let result: AnyPublisher<OperationResult, Never> =
            source
