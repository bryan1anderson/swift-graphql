...
final class CacheExchangeTests: XCTestCase {
    ...
    func environment(
        ...
        await withCheckedContinuation { continuation in
            ...
            let queue = DispatchQueue(label: "com.swiftgraphql.test.trace")
            var trace: [String] = []
            ...
            let completion = exchange.register(
                client: client,
                operations:
                    operations
            ...
            operations.send(completion: .finished)
    ...
    func testOnQueryCacheFirstHitDoesNotForwardRequest() async throws {
        let trace = await environment { operations, results in
    ...
    func testOnQueryCacheFirstForwardsMissingRequest() async throws {
        let trace = await environment { operations, results in
    ...
    func testCacheAndNetworkForwardsRequest() async throws {
        let trace = await environment { operations, results in
    ...
    func testCacheOnlyDoesntForwardRequest() async throws {
        let trace = await environment { operations, results in
    ...
    func testNetworkOnlyForwardRequest() async throws {
        let trace = await environment { operations, results in
    ...
    func testOnMutationDoesNotCache() async throws {
        let trace = await environment { operations, results in
    ...
    func testMutationInvalidatesQueries() async throws {
        let trace = await environment { operations, results in
            let op = CacheExchangeTests.queryOperation.with(policy: .cacheAndNetwork)
    ...
    func testOnSubscriptionForwardsSubscription() async throws {
        let trace = await environment { operations, results in
